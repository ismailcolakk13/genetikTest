...
def ucak_govdesi_olustur(aircraft):
    ...
    traces.append(go.Mesh3d(
        x=x_h, y=y_h, z=z_h,
        color='lightblue', opacity=0.5, name='Yatay Kuyruk',
        i=[0, 0], j=[1, 2], k=[2, 3]
    ))
    ...
    traces.append(go.Mesh3d(
        x=x_v, y=y_v, z=z_v,
        color='lightblue', opacity=0.5, name='Dikey Kuyruk',
        i=[0, 0], j=[1, 2], k=[2, 3]
    ))
...
# ---------------------------------------------------------------------------
# ANA GÖRSELLEŞTİRME  — açık tema (eski)
# ---------------------------------------------------------------------------
...
def gorsellestir_tasarim(en_iyi_tasarim, best_score, best_cg, aircraft, ALGORITMA):
    ...
    for k_id, pos in en_iyi_tasarim.yerlesim.items():
        komp = next((c for c in aircraft.komponentler_db if c.id == k_id), None)
        boyut = komp.boyut if komp else (10, 10, 10)
        idx = aircraft.komponentler_db.index(komp) if komp else 0
        color = KOMPONENT_RENKLERI.get(k_id, colors[idx % len(colors)])

        for trace in komponent_ciz(k_id, pos, boyut, color):
            fig.add_trace(trace)
